...
export const getBaseTotal = (items: Item[]) => items.reduce((acc, item) => acc + getItemBaseTotal(item, item.amount), 0)

export const getPartyTotal = (items: Item[]) => getBaseTotal(items) - getTotalDiscount(items)

export const getPartyUserBaseTotal = (items: Item[], id: string) => items
    .reduce((acc, item) => {
        const participants = getItemParticipants(item);
        const userIndex = participants.findIndex((user) => user.id === id);
        if (userIndex >= 0) {
            return acc + (item.equally ? getItemBaseTotal(item, item.amount) / participants.length : getItemBaseTotal(item, item.users[userIndex].value));
        }
        return acc;
    }, 0)

export const getPartyUserDiscount = (items: Item[], id: string) => items
    .reduce((acc, item) => {
        const participants = getItemParticipants(item);
        const userIndex = participants.findIndex((user) => user.id === id);
        if (userIndex >= 0) {
...
export const getPartyUserTotal = (items: Item[], id: string) => getPartyUserBaseTotal(items, id) - getPartyUserDiscount(items, id)

export const splitItems = (items: Item[], userId: string): [Array<
    Item & { originalIndex: number; originalUserIndex: number; total: number; participants: number }
